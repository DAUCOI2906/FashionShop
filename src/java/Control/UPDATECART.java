...
public class UPDATECART extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String cid_w = request.getParameter("idbill");
        String iduser_w = request.getParameter("iduser");
        try {
            PrintWriter out = response.getWriter();
            ProcessData ps = new ProcessData();
            Integer cid = Integer.parseInt(cid_w);
            Integer iduser = Integer.parseInt(iduser_w);
            int k = 0;
            DETAILSCART cart = ps.GETONECART(cid);
            SANPHAM sp = ps.GETPRODUCTBYIDBILL(cart.getIdbill());
            request.setAttribute("product", sp);
            request.setAttribute("idbill", cart.getIdbill());
